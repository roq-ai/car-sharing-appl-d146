...
import { useRoqClient, useCompanyFindManyWithCount } from 'lib/roq';
...
export function CompanyListPage(props: CompanyListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useCompanyFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.company.delete({ where: { id } });
      await mutate();
    } catch (error) {
