...
import { useRoqClient, useMaintenanceFindFirst } from 'lib/roq';
...
function MaintenanceEditPage() {
  ...
  const { data, error, isLoading, mutate } = useMaintenanceFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: MaintenanceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.maintenance.update({
        data: values as RoqTypes.maintenance,
        where: {
          id,
        },
      });
