...
import { useRoqClient, useMaintenanceFindManyWithCount } from 'lib/roq';
...
export function MaintenanceListPage(props: MaintenanceListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['type.contains', 'description.contains'],
          ...params.filters,
        },
        'maintenance',
      ),
  ...
  const { data, error, isLoading, mutate } = useMaintenanceFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.maintenance.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    { id: 'type', header: 'Type', accessorKey: 'type' },
    {
      ...
      header: 'Date',
      accessorKey: 'date',
      cell: ({ row: { original: record } }: any) => (record?.date ? format(record?.date, 'dd-MM-yyyy') : ''),
    },
